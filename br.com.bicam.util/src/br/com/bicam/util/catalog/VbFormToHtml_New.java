...
//import br.com.bicam.util.symboltable.Scope;
import br.com.bicam.util.symboltable.Symbol_New;

public class VbFormToHtml_New {
...
/*   private static HashMap<String,String> weightMap = new HashMap<String,String>(){
	   { put("700","bold");
	     put("400","normal");
	   }
   };*/
   
/*   private static HashMap<String,String> textAlignMap = new HashMap<String,String>(){
	   { put("0","left");
	     put("1","right");
	     put("2","center");
	   }
...
/*   private static HashMap<String,String> propertyMap = new HashMap<String,String>(){
	   { put("ClientTop","margin-top");
	     put("ClientLeft","margin-left");
	     put("Top","margin-top");
	     put("Left","margin-left");	     
	     put("ClientHeight","height");
	     put("ClientWidth","width");
	     put("Height","height");
	     put("Width","width");
	     put("Caption","");	     
	     put("BackColor","background-color");
	     put("CONTROL","background-color");
	   }
   };*/
   
   public VbFormToHtml_New(String groupTemplates, SymbolTable_New _symTable){
	   	 symTable = _symTable;
	     templates = new STGroupFile(groupTemplates); // load templates group
		 hierarchyChainST = new Stack<ST>();
		 htmlFiles = new HashSet();
   ...
   private void run(){
	   ...
	   // Carrega de symbolTable os forms
	   ...
	   Set<String> visitedSymbol = new HashSet<String>();
	   ...
	   if(sym.hasProperty("Index", "-1"))  continue; //  ARRAY DEFINIDO IMPLICITAMENTE
	   if(visitedSymbol.contains(sym.getName())) continue; // Existe o mesmo symbol em diferentes escopos

	   visitedSymbol.add(sym.getName());
//	   listMembers((IScope_New)sym);
...
/*   private void runFrame(Scope _frame){
	       clearST();
		   listMembers(_frame);
		   System.err.println("Building web Frame page: " + _frame.getName());
		   clearST();
		   rootST = templates.getInstanceOf("root");
		   pushST(rootST);
		   htmlDocFrame((Symbol)_frame);
		   System.err.println("\nWeb Frame Page...\n" + rootST.render());
		   String fileName = _frame.getName() + ".html";
		   try {
			sendHtmlToFile(fileName);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
...
/*   private void listMembers(IScope_New scope){
	   for(BCString bc : scope.getMembers().keySet()){
		   System.err.println("Member: " + bc.toString());
	   }
   }*/
...
/*   private void htmlDocFrame(Symbol form){
   ...
   private void head(Symbol_New form){
	   ...
	   attributeST.add("attribValue", "ISO-8859-1");	   
	   popST(); // ends meta
...
//	   properties.put("background-color", "#FFFFC0");
	   ...
	   if(_sym.hasProperty("CONTROL", "Form")) {
		   scp = (IScope_New)_sym;
	   }
	   
	   if(scp == null){
		   if(_sym.hasProperty("CONTROL", "MDIForm")) {
			   scp = (IScope_New)_sym;
		   }
	   }
...
//		   scp = ((IScope_New)_sym).getScopeByProperty("CONTROL", "Form");
//		   IScope_New scp = null;
		   scp = null;
		   if(_sym.hasProperty("CONTROL", "Form")) {
			   scp = (IScope_New)_sym;
		   ...
		   if(scp == null){
			   if(_sym.getProperty("CONTROL") != null){
				   if (_sym.hasProperty("CONTROL", "CommandButton")){
					   ...
					   String cap = ((String)_sym.getProperty("Caption")).replace("\"", "");
					   cap = montaShortCut(cap,"&");
//					   peekST().add("content",((String)_sym.getProperty("Caption")).replace("\"", ""));
					   peekST().add("content",cap);
				   }
			   }
		   }
	   }
...
//			   properties.put("border-style", "hidden");
   ...
   private void fillBody(IScope_New compoment){
	   for(StringOptionalCase bc : compoment.getMembers().keySet()){
		   Symbol_New sym = compoment.getMembers().get(bc).getFirstSymbol();

		   if(sym.hasProperty("DEF_MODE","IMPLICIT")) continue; // não trata symbolos definidos implicitamente
		                                                         // como visible, no commando "<label>.Visible = True"

		   if(sym.hasProperty("Index","-1")) continue; // Veja DefSymbol class
		   
		   if(!sym.getEnclosingScope().equals(compoment)) continue;
		   
		   if(!visited.contains(sym.getName()) ){
			   visited.add(sym.getName());
		   }
			else continue;
/*		   if(sym.hasProperty("CONTROL","SSFrame")) continue; 
		   
		   if(sym.hasProperty("CONTROL","SSPanel")) continue;*/ 
		   
//		   if(sym.getName().contains("#")) continue; // não trata symbolos de array 
...
//	   hierarchyChainST.add(new ST[]{stack,null,null}); // ST[3]={stack,attribute,content}
