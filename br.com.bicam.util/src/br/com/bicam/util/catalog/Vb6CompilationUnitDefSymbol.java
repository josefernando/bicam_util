...
import br.com.bicam.util.datamodel.Vb6Form;
import br.com.bicam.util.datamodel.Vb6Module;
...
public class Vb6CompilationUnitDefSymbol extends VisualBasic6BaseListener {
...
//	CompilationUnit compilationUnit;
	...
	public Vb6CompilationUnitDefSymbol(PropertyList _propertyList) {
...
//		String name = compilationUnitName.split("\\.")[0]; // remove extenção
		...
		if(Vb6Form.class.isInstance(appComponent)) {
//		if (name.equalsIgnoreCase(vbName) && compilationUnit.getFileName().toUpperCase().endsWith("FRM") ) {
			prop2.addProperty("NAME", vbName + "#COMPILATION_UNIT"); // "#COMPILATION_UNIT# diferencia compUnitName
		} 
//			else {
//			if(compilationUnit.getFileName().toUpperCase().endsWith("FRM")){
//				prop2.addProperty(NAME, name); // Form file tem nome diferente do form 
//			}
			else prop2.addProperty(NAME, vbName);
//		}	
		
		prop2.addProperty(MODULE_NAME, vbName);
...
//		prop2.addProperty("COMPILATION_UNIT_NAME", compilationUnitName);
		prop2.addProperty("APP_COMPONENT", appComponent);
...
//		if (!compilationUnitName.split("\\.")[1].equalsIgnoreCase("FRM")) {
		if(Vb6Form.class.isInstance(version))   {
...
//				}
//			}
			Object appComponent =  symModuleType.getProperty("APP_COMPONENT");
			if(appComponent == null) {
//				Exception e = new Exception();
				BicamSystem.printLog("ERROR", "COMPONENT NOT FOUND IN COMPILATION UNIT");
			}
			else if (Vb6Form.class.isInstance(appComponent) ||  // .bas e .clas tem métodos publicos 
					Vb6Module.class.isInstance(appComponent)) {
					properties.addProperty(VISIBILITY_SCOPE, st.getPublicScope()); // publicScope);
					properties.addProperty(VISIBILITY_SCOPE_NAME, st.getPublicScope().getName()); // publicScope.getName());
			}			
		}
		...
		if (NodeExplorer.getAncestorClass(varCtx, "FormalParameterContext") != null) {
			...
			try {
				...
				System.err.format(
						...
						defaultProperties().getProperty(NAME), varCtx.start.getLine(), version.getId());
				e.printStackTrace();
			}
			return "UNDEFINED";
		}
